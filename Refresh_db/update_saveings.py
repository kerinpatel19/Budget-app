...
class Update_saveings_account:
    @classmethod
    def Update_Saveing_account(cls, db_host, db_user, db_password, db_name, table_name, year):
        ...
        # Fetch the row
        row = cursor.fetchone()
        daybefore_Saveing_account = float(row[4])
        ...
        while current_date <= datetime.strptime(end_date, "%Y-%m-%d"):
            ...
            Insert_Saveing_account = daybefore_Saveing_account + Savings_AJC
            
            # Construct and execute the INSERT query
            insert_query = f"""
                INSERT INTO {table_name} (date,Savings)
                VALUES (%s,%s)
            ...
            Insert_Saveing_account,
            )
            
            daybefore_Saveing_account = Insert_Saveing_account
            try:
